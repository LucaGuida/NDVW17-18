...
public class gamelogic : MonoBehaviour {
	...
	public GameObject RSPCamera;

	GameObject player1;
	PlayerScript player1Script;
	public GameObject[] player1Blocks = new GameObject[3];
	
	GameObject player2;
	PlayerScript player2Script;
	public GameObject[] player2Blocks = new GameObject[3];
	...
	int[,] winLooseMatrix = new int[3, 3] {
			// Rock, Scissor, Paper
	   /*Rock*/{ 0, 2, 1 }, 
	/*Scissor*/{ 1, 0, 2 },
	  /*Paper*/{ 2, 1, 0 }
	...
	private float DELAY_BETWEEN_ROUNDS = 3.0f;
	saveData dataObject;
	...
	void Start () {
		...
		GameObject.Find("ScoreNeeded").GetComponent<TextMesh>().text = "Score needed to win: " + ScoreRequiredToWin;
	...
	void Update () {
		if (shouldRestart && timeFinish < 0) {
			shouldRestart = false;
			timeFinish = DELAY_BETWEEN_ROUNDS;
			Reset ();
	...
	void StartNewMatch(){
		//If game has ended, go back to the maze
//		string sceneName = PlayerPrefs.GetString("demo");
//		SceneManager.LoadScene(sceneName);//back to demo scene
		MazeSpawner.decrementNumberOfCoins();

		player1Script.resetScores ();
		player2Script.resetScores ();

		// set score required to win for next game, assuming that 15 is the max number of coins placed in the maze at the beginning of the game
		ScoreRequiredToWin = 15 - MazeSpawner.getNumberOfCoins() + 2 ;
		GameObject.Find("ScoreNeeded").GetComponent<TextMesh>().text = "Score needed to win: " + ScoreRequiredToWin;
	...
	void checkEndOfGame(){
		...
		if(player1Script.OverallScore >= ScoreRequiredToWin){
			resultText.GetComponent<TextMesh>().text = "You won the game";
		...
		else if(player2Script.OverallScore >= ScoreRequiredToWin){
			resultText.GetComponent<TextMesh>().text = "AI won the game";
	...
	void giveBonusScore(int bonusScoreToGive, PlayerScript toThisPlayer){
		...
		if(bonusScoreToGive==1){
			comboText.text = "Combo! 1 extra point!";
		}else if(bonusScoreToGive==2){
			comboText.text = "MEGA Combo! 2 extra points!";
		}
	}
	...
	void player1WonRound(){
		...
		resultText.GetComponent<TextMesh>().text = "You won!";
	}
	...
	void player2WonRound(){
		...
		resultText.GetComponent<TextMesh>().text = "AI won!";
	}
	
	void nooneWonRound(){
		//Debug.Log("Round is draw");
		currentRoundResult = RoundResult.draw;
		resultText.GetComponent<TextMesh>().text = "Round is draw";
	}
	
	public void Reset(){
		if(!roundActive){
			//resultText.GetComponent<TextMesh>().text = "Result:";
			resultText.GetComponent<TextMesh>().text = "It's your turn!";
